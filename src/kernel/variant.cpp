...
namespace kernel {
    ...
    void Variant::free() {
        ...
        case Type::BYTE_ARRAY:
            delete _M_byte_array_P;
            break;
        case Type::REGEX:
            delete _M_regex_P;
            break;
        case Type::DATE_TIME:
            delete _M_date_time_P;
            break;
    ...
    string Variant::toString() const {
        ...
        case Type::BYTE_ARRAY: {
            string_os << "[bytes: ";
            for (size_t i = 0;
                 i < std::min(size_t(10), _M_byte_array_P->size()); ++i) {
                if (i > 0)
                    string_os << ", ";
                string_os << std::hex << (int)(*_M_byte_array_P)[i];
            }
            if (_M_byte_array_P->size() > 10)
                string_os << ", ...";
            string_os << "]";
            return string_os.str();
        }
        case Type::DATE_TIME: {
            auto time = std::chrono::system_clock::to_time_t(*_M_date_time_P);
...
#define CALCULATE_INT_IMPL(fname, op)                                          \
    ...
    Ref<Variant> Variant::fromStringView(const std::string_view &value) {
        Ref<Variant> res = new Variant();
        res->_M_type = Type::STRING_VIEW;
        res->_M_string_P = new std::string(value);
        return res;
    }

    Ref<Variant> Variant::fromByteArray(const std::vector<uint8_t> &value) {
        Ref<Variant> res = new Variant();
        res->_M_type = Type::BYTE_ARRAY;
        res->_M_byte_array_P = new std::vector<uint8_t>(value);
        return res;
    }

    Ref<Variant>
    Variant::fromDateTime(const std::chrono::system_clock::time_point &value) {
        Ref<Variant> res = new Variant();
        res->_M_type = Type::DATE_TIME;
        res->_M_date_time_P = new std::chrono::system_clock::time_point(value);
        return res;
    }

    Ref<Variant> Variant::fromRegex(const std::regex &value) {
        Ref<Variant> res = new Variant();
        res->_M_type = Type::REGEX;
        res->_M_regex_P = new std::regex(value);
        return res;
    }
