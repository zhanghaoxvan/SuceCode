...
namespace kernel {
    const std::array<byte, 8> Sout::query{
        0x42, 0x2a, 0x4b, 0xf0, 0x7a, 0x37, 0x37, 0x37,
    };

    bool Sout::isSout(const vector<byte> &bytes) {
        if (bytes.size() < 8)
            return false;
    ...
    bool Sout::isSout(const string &path) {
    ...
    Function Sout::load(const string &path) {
        std::ifstream is(path, std::ios::binary);
    ...
    Function Sout::load(const vector<byte> &bytes) {
        auto it = bytes.cbegin() + query.size();
        return Deserialize<Function>()(it);
    }
    vector<byte> Sout::dump(const Function &func) {
