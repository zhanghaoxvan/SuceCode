...
namespace kernel {
    namespace token {
        ...
        string toString(token::tag_t tag);  // 添加命名空间限定

        class Token {
          private:
            token::Tag _M_tag;  // 添加命名空间限定
            uinteger _M_line;
            Ref<string> _M_chunk;
...
namespace std {
    template <> struct hash<kernel::token::Token> {
        size_t operator()(const kernel::token::Token &token) const {
            return token.hash();
        }
    };

    template <> struct equal_to<kernel::Ref<kernel::token::Token>> {
        bool operator()(const kernel::Ref<kernel::token::Token> &a,
                        const kernel::Ref<kernel::token::Token> &b) const {
            return kernel::token::equals(*a, *b);
        }
    };
} // namespace std
